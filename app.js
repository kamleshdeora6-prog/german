...
const LS_KEY='german_app_replacement_v1';
...
async function init(){try{
 const [passages,notes,exercises,flashcards,conjugation,sentences,vocab]=await Promise.all([loadJson('passages.json'),loadJson('notes.json'),loadJson('exercises.json'),loadJson('flashcards.json'),loadJson('conjugation.json'),loadJson('sentences.json'),loadJson('vocab.json')]);
...
function renderPassages(){const level=q('#passageLevel').value||'All'; const term=(q('#passageSearch').value||'').toLowerCase(); let list=state.data.passages.filter(p=>(level==='All'||(p.level||'').toUpperCase()===level) && (!term || [p.title,p.chapter,p.german,p.english].join(' ').toLowerCase().includes(term))); q('#passagesList').innerHTML=list.map((p,i)=>`<div class="card"><div class="row" style="justify-content:space-between"><div><span class="pill">${p.level}</span> <strong>${p.title||'Untitled passage'}</strong><div class="small muted">${p.chapter||''}</div></div><div class="row"><button onclick="listenText(${i})">🔊 Listen</button><button onclick="markPassageRead(${i})">✅ Read</button></div></div><pre style="white-space:pre-wrap">${escapeHtml(p.german||'')}</pre>${p.english?`<details><summary>English</summary><pre style="white-space:pre-wrap">${escapeHtml(p.english)}</pre></details>`:''}</div>`).join('') || '<div class="card">No passages found.</div>';}
window.listenText=(i)=>speak(state.data.passages[i]?.german||''); window.markPassageRead=(i)=>{ensureToday(); store.daily[todayKey()].passages++; saveStore(); updateMissionUI(); buildInsights(); alert('Passage counted for today.');};
function speak(text){if(!('speechSynthesis' in window)) return alert('Speech is not supported in this browser.'); const u=new SpeechSynthesisUtterance(text); u.lang='de-DE'; speechSynthesis.cancel(); speechSynthesis.speak(u);}
// Exercises
